        ...
        for x in range(0, len(lDb)):
            if lDb[x].startswith("p:"):
                ...
                canalclean = re.sub(re.compile('\W'), '', ''.join(c.lower() for c in unicodedata.normalize(
                    'NFKD', str(nomeCanal).replace("+", "plus")).encode('ascii', 'ignore').decode('ascii') if not isinstance(c, int) and not c.isspace()))
            ...
            for line in f:
                if line.startswith("#SERVICE"):
                    ...
                    if "::" in lineSpl:
                        ...
                        canalclean = re.sub(re.compile('\W'), '', ''.join(c.lower() for c in unicodedata.normalize(
                            'NFKD', tmpChannel[1].replace("+", "plus")).encode('ascii', 'ignore').decode('ascii') if not isinstance(c, int) and not c.isspace()))
...
def getTvhChannelList(conf):
    ...
    for l in listURL['entries']:
        canalclean = re.sub(re.compile('\W'), '', ''.join(c.lower() for c in unicodedata.normalize(
            'NFKD', l['val'].replace("+", "plus")).encode('ascii', 'ignore').decode('ascii') if not isinstance(c, int) and not c.isspace()))
